...
def train(dataset, alpha, beta, thr):
    ...
    for i in np.arange(steps):
        ...
        if i != 0 and h_quality.history['val_acc'][-epochs_training-1] - np.min(h_quality.history['val_acc'][-epochs_training:]) > 0.2:
          model = clone_model(model_quality)
          model.set_weights(model_quality.get_weights())
          
    s_bnn = 0
    val_accc = h_quality.history['val_acc']
    accc = h_quality.history['acc']
